...
# === Инициализация БД (запуск при старте gunicorn) ===
def init_db():
...
# === Запуск БД при старте ===
init_db()  # ← ВАЖНО: запускается при gunicorn
...
<script>
    ...
    // Другие функции (каналы, эмодзи и т.д.) можно добавить позже
</script>
</body></html>'''
